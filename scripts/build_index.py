...
def build_index():
    ...
    if not os.path.exists(base_file):
        raise FileNotFoundError(f"Base vectors file {base_file} not found. Run generate_data.py first.")
    ...
    base_data = np.memmap(base_file, dtype=np.float32, mode='r', offset=8, shape=(num_vectors, dim))
