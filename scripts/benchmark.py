...
def run_benchmarks():
    ...
    if has_torch or has_faiss:
        ...
        base_vectors = np.memmap(
            base_file, dtype=np.float32, mode="r", offset=8, shape=(n_base, dim)
        )

        query_hdr = np.fromfile(query_file, dtype=np.uint32, count=2)
        n_query = int(query_hdr[0])
        query_vectors = np.memmap(
            query_file, dtype=np.float32, mode="r", offset=8, shape=(n_query, dim)
        )
        ...
        if has_torch:
            ...
            if torch.cuda.is_available():
                try:
                    ...
                    end_time = time.time()
                    pytorch_latency = (
                        (end_time - start_time) / num_queries
                    ) * 1000.0  # ms
        ...
        if has_faiss:
            ...
            try:
                ...
                centroids = np.fromfile(
                    centroids_file, dtype=np.float32, offset=8
    ...
    if naive_results and hybrid_results:
        ...
        for q in range(min(num_queries, 5)):
            if q in naive_results and q in hybrid_results:
                ...
                print(
                    f"Query {q}: Naive IDs={sorted(list(naive_ids))}, Hybrid IDs={sorted(list(hybrid_ids))}, Recall={recall * 100:.1f}%"
                )
        if recalls:
            print(
                f"Average top-{k} Recall (IVF vs Exact CPU): {np.mean(recalls) * 100:.1f}%"
            )
